...
def parse(content, strict=False):
    ...
    for line in string_to_lines(content):
        ...
        if line.startswith(protocol.ext_x_byterange):
            ...
            state['current_cue_out_scte35'] = line.split(':', 1)[1]

        elif line.startswith(protocol.ext_x_cue_start):
            if line.startswith(protocol.ext_x_cue_out):
                _parse_cueout(line, state)
                state['cue_out'] = True
            else:
                state['cue_start'] = True
                state['cue_out'] = True
                state['current_cue_out_duration'] = line.split(':', 1)[1]
...
def _parse_ts_chunk(line, data, state):
    ...
    if state.get('current_cue_out_scte35'):
        segment['scte35'] = state.pop('current_cue_out_scte35', False)
        segment['scte35_duration'] = state.pop('current_cue_out_duration', False)
...
def _parse_cueout(line, state):
    param, value = line.split(':', 1)
    res = re.match('.*ElapsedTime=(.*),Duration=(.*),SCTE35=(.*)$', value)
    if res:
        state['current_cue_out_elapsedtime'] = res.group(1)
        state['current_cue_out_duration'] = res.group(2)
        state['current_cue_out_scte35'] = res.group(3)
